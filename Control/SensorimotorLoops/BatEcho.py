...
class AvoidApproach(Avoid):
    ...
    def _calc_angular_velocity(self, v, R):
        if R!=0: return v/R
        else: return config.MAX_ANGULAR_VELOCITY
    ...
    def _calc_turning_radius(self, iid, v, onset_distance):
        ...
        approach_term = -np.sign(iid)*(np.log(np.abs(scaled_IID))-R_min) if scaled_IID!= 0 else sys.float_info.max if np.random.rand() > 0.5 else -sys.float_info.max
        ...
        avoid_term = -np.sign(iid)*(R_min - np.log(1-scaled_onset)) if scaled_onset<1 else sys.float_info.max if np.random.rand() > 0.5 else -sys.float_info.max
...
class AvoidApproachCruise(Avoid):
    def __init__(self, background=None, **kwargs):
        super().__init__(background)
        self.steer_damper = config.APPROACH_STEER_DAMPING
        self.approach_factor = 0 if 'approach_factor' not in kwargs.keys() else kwargs['approach_factor']
    ...
    def _calc_turning_radius(self, iid, v, onset_distance):
        ...
        if A == 2: 
            #iid = self.IID_QUIET_RAND * np.random.rand() - 0.5*self.IID_QUIET_RAND
            #A = 1
            return sys.float_info.max if np.random.rand() > 0.5 else -sys.float_info.max
