...
from Control.SensorimotorLoops.BatEcho import AvoidApproach, AvoidApproachCruise
...
def run_1_primer_episode(time_limit=TIME_LIMIT):
    ...
    for _ in range(time_limit):
        ...
        if Helper.collision_check(inview, 'plant') or Helper.collision_check(inview, 'pole'):
            ...
            _,_ = poses.pop(), compresses.pop()
            break
        action, zone = Helper.behaviorCruise(pose, beacons=beacons, classifier=cls,
                                      margin=MARGIN, scan_range=SCAN_RANGE)
        zones.append(zone)
    ...
    while (result=='hit'):
        ...
        for N in [abs(kth)-1]+list(range(abs(kth))):
            ...
            while (i<N_replay):
                ...
                else: action, zone = Helper.behaviorCruise(pose, beacons=beacons, classifier=cls,
                                       margin=MARGIN, scan_range=SCAN_RANGE)
...
def main():
    ...
    if RUN_ID==MAX_RUN:
        ...
        df.to_pickle(f'./labeled_echo_data/run_CRUISE_{DATE}.pkl')
